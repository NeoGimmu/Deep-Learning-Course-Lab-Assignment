...
def main():
    ...
    print("\n2. 构建MLP 模型...")
    model = MLP()
    # 网络结构：输入层(13) → 隐藏层1(64, ReLU) → 隐藏层2(32, ReLU) → 输出层(1, Linear)
    model.add_layer(Dense(input_dim=13, output_dim=128,
    weight_initializer='he'))
    model.add_layer(ReLU())
    model.add_layer(Dense(input_dim=128, output_dim=64,
    weight_initializer='he'))
    model.add_layer(ReLU())
    model.add_layer(Dense(input_dim=64, output_dim=1,
    weight_initializer='he'))
    model.add_layer(Linear())
    # 设置损失函数和优化器
    model.set_loss(MSE())
    model.set_optimizer(Adam(learning_rate=0.003))
    #model.set_optimizer(SGD(learning_rate=0.003))
    print(" 模型结构：13 → 128(ReLU) → 64(ReLU) → 1(Linear)")
    print(" 损失函数：MSE")
    print(" 优化器：Adam (lr=0.003)")
    print("=" * 50)
    # 3. 训练模型
    print("\n3. 开始训练模型...")
    history = model.train(X_train=X_train,y_train=y_train,epochs=100,batch_size=32,validation_data=(X_test, y_test))
    print("=" * 50)
    # 4. 评估模型性能
    print("\n4. 评估模型性能...")
    test_loss, metrics = model.evaluate(X_test, y_test)
    print(f" 测试集MSE 损失：{test_loss:.4f}")
    print(f" 平均绝对误差(MAE)：{metrics['mae']:.2f}（千美元）")
    print(f" R²评分：{metrics['r2']:.4f}（越接近1 越好）")
    print("=" * 50)
    # 5. 可视化结果
    print("\n5. 可视化训练结果...")
